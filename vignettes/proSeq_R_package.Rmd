...
Let's check the respective files.
...
head(meta)
```
And the counts matrix is like this:
```{r}
countsMatrix[1:5,1:2]
```
...
In our case, the number of cells that failed the QC is 10.
...
You can immediately see that the fit is not as good and that I had to filter more values with high CV2, as they biased the fit.
